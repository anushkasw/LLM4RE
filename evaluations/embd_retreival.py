...
def main(args):
                ...
                if type(triple[0]) == list:
                    for triple_ in triple:
                        for element in triple_:
                            if embd_list:
                                if element.strip() not in embd_list:
    ...
    for data in ['NYT10', 'tacred', 'crossRE', 'FewRel']:
        print(data)
        if args.exp == 'JRE':
            data_dict, _ = get_JRE_data(data)
        else:
            data_dict, _ = get_RC_data(data)

        for dict_ in data_dict.values():
            if args.exp == 'JRE':
                triple_list = dict_['triples']
                for triple in triple_list:
                    if len(triple[0]) > 0:
                        triple[1] = triple[1].replace('_', ' ').replace('/', ' ').replace("-", " ").strip()
            else:
